...
Feature: Default
	...
	Scenario: Creating a poll by adding questions and multiple answers
		Given User is on Activity stream page
				...
				And User types a question in the question box
				And User clicks Answer 1 box 
				And User types the Answer 1 into Answer1 box
		  		And User clicks Answer 2 box 
				And User types the Answer 2 into Answer2 box
	...
	Scenario: Verifying the 'The message title is not specified' error message
		Given User is on Activity stream page
				When User clicks POLL tab
				And User clicks the SEND button to send empty message 
	...
	Scenario: Verifying the 'Please specify at least one person.' error message
		Given User is on Activity stream page
				When User clicks POLL tab
				And User click to default All employees button to delete it
				And User clicks the SEND button to send empty message 
	...
	@B32G17-221
	Scenario: Verifying the 'The question "<What is your favorite language?>" has no answers.' error message
		Given User is on Activity stream page
				When User clicks POLL tab
				And User clicks message box to type a message
				And User types a message
				...
				And User types a question in the question box
				And User clicks the SEND button to send the message
				Then User sees error message "<What is your favorite language?>" has no answers.	
